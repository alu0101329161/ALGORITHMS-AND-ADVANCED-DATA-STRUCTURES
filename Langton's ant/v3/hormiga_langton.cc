...
int main (int argc, char* argv[]) {
    ...
    Hormiga aux;
    vector_hormigas.push_back(aux);

    for(int recorrido = cantidad_hormigas; recorrido > 1; recorrido --) {
     std::cout << "Introduzca posicion hormiga"<< std::endl;
     std::cin >> aux_x;
     std::cin >> aux_y;
     aux.Resize(aux_x, aux_y);
     vector_hormigas.push_back(aux);
    }
